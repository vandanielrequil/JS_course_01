...
//*************Добавдение вывода счёта */

var mainBlock = document.querySelector('.wrap'),
    scoreDisp = document.createElement('div'),
    scoreDispText = document.createElement('h3'),
    scoreDispScore = document.createElement('h3');

scoreDisp.classList.add('scoreDisplay');
scoreDispText.innerHTML = 'Ваш счёт:&nbsp;';
scoreDispScore.innerText = score;
scoreDispScore.id = 'scoreDispScore';

scoreDisp.append(scoreDispText);
scoreDisp.append(scoreDispScore);

mainBlock.insertAdjacentElement('afterbegin', scoreDisp);
...
function move() {
    ...
    // 3) Нет ли препятствия
    //console.log(new_unit);
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !isStoneUnit(new_unit)) {
 ...
 * @returns {boolean}
 */
function isStoneUnit(new_unit) {
    var check = false;

    if (new_unit.classList.contains('stone-unit')) {
        check = true;
    }
    return check;
}
...
function createStone() {
    ...
    while (!stoneCreated) { //пока препятствий не создали
        // рандом
